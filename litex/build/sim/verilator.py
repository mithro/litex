...
def _build_tb(platform, vns, serial, template, extra_cpp_files):
...
""".format(
    ...
    try:
        ios += """
#define WITH_JTAG
#define JTAG_TMS dut->{jtag_tms}
#define JTAG_TDI dut->{jtag_tdi}
#define JTAG_TCK dut->{jtag_tck}
#define JTAG_TDO dut->{jtag_tdo}
""".format(
    jtag_tms=io_name("jtag", "tms"),
    jtag_tdi=io_name("jtag", "tdi"),
    jtag_tck=io_name("jtag", "tck"),
    jtag_tdo=io_name("jtag", "tdo"),
    )
    except:
        pass
...
def _build_sim(platform, vns, build_name, include_paths, serial, verbose):
    ...
    extra_cpp_files = []
    for source, language, library in platform.sources:
        if language == "cpp":
            filename = os.path.basename(source)
            tools.write_to_file(filename, open(source, "r").read())
            extra_cpp_files.append(filename)
...
""".format(
    ...
    _build_tb(platform, vns, serial, os.path.join(sim_directory, "dut_tb.cpp"), extra_cpp_files=extra_cpp_files)
